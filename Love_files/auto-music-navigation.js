// 自动音乐导航系统 - page2也自动播放
class AutoMusicLoveNavigation {
    constructor() {
        this.pages = [
            { id: 'index', title: '爱的序章', file: 'index.html', autoMusic: true },
            { id: 'page2', title: '甜蜜报告', file: 'page2.html', autoMusic: true },
            { id: 'page3', title: '聊天解析', file: 'page3.html', autoMusic: false },
            { id: 'page4', title: '回忆相册', file: 'page4.html', autoMusic: false },
            { id: 'page5', title: '爱情时间线', file: 'page5.html', autoMusic: false },
            { id: 'page6', title: '永恒誓言', file: 'page6.html', autoMusic: false }
        ];
    ...
    showPageWelcomeMessage() {
        ...
        if (this.currentPage === 0) {
            ...
            // page2：自动播放音乐
            if (!this.musicStarted) {
                this.showTooltip('📊 欢迎查看甜蜜聊天报告！（音乐将自动播放）');
                setTimeout(() => {
                    this.autoStartMusic();
                }, 2000);
            } else if (this.musicPlaying) {
                this.showTooltip('📊 继续浏览甜蜜聊天报告 💕');
            } else {
                this.showTooltip('📊 欢迎查看甜蜜聊天报告！点击音乐按钮播放 🎵');
            }
        } else {
            // 其他页面：正常提示
            if (this.musicPlaying) {
                this.showTooltip(`欢迎来到${currentPage.title} 💕`);
            } else if (this.musicStarted) {
                this.showTooltip(`欢迎来到${currentPage.title}！音乐已暂停 🎵`);
            } else {
                this.showTooltip(`欢迎来到${currentPage.title}！点击音乐按钮开始 🎵`);
            }
        }
    }
                    ...
                    <div class="page-progress">
                        ${this.pages.map((page, index) => `
                            <div class="progress-dot ${index === this.currentPage ? 'active' : ''} ${index < this.currentPage ? 'completed' : ''}"
                                 ...
                                 data-index="${index}"
                                 title="${page.title}"
                                 onclick="window.autoMusicNavigation.goToPage(${index})">
                            </div>
                        `).join('')}
                    </div>

                    <button class="nav-btn prev-btn" ${this.currentPage === 0 ? 'disabled' : ''} onclick="window.autoMusicNavigation.goToPage(${this.currentPage - 1})">
                        上一页
                    </button>

                    <div class="page-title">${this.pages[this.currentPage].title}</div>

                    <button class="nav-btn next-btn" ${this.currentPage === this.pages.length - 1 ? 'disabled' : ''} onclick="window.autoMusicNavigation.goToPage(${this.currentPage + 1})">
                        下一页
                    </button>

                    <button class="music-toggle ${this.musicPlaying ? 'playing' : ''}" title="背景音乐" onclick="window.autoMusicNavigation.toggleMusic()">
                        🎵
                    </button>
                </div>
            </nav>
        ...
        try {
            ...
            if (musicEverStarted) {
                ...
                if (this.musicPlayer) {
                    ...
                    if (wasPlaying) {
                        setTimeout(() => {
                            this.attemptMusicPlay();
                        }, 1000);
                    }
                }
            } else {
                console.log('音乐尚未启动，等待触发...');
                this.musicStarted = false;
                this.musicPlaying = false;
            }
    ...
    attemptMusicPlay() {
        // 尝试播放音乐（用于恢复播放状态）
        if (this.musicPlayer && this.musicStarted && !this.musicPlaying) {
            this.musicPlayer.play().then(() => {
                console.log('恢复音乐播放成功');
            }).catch(error => {
    ...
    getMusicState() {
        return {
            ...
            currentPage: this.pages[this.currentPage].title,
            autoMusic: this.pages[this.currentPage].autoMusic
        };
    }
}

// 页面加载完成后初始化
let autoMusicNavigationInstance = null;

document.addEventListener('DOMContentLoaded', () => {
    console.log('DOM加载完成，初始化自动音乐导航系统...');

    // 防止重复初始化
    if (autoMusicNavigationInstance) {
        autoMusicNavigationInstance.createNavigation();
        return;
    }

    autoMusicNavigationInstance = new AutoMusicLoveNavigation();
    window.autoMusicNavigation = autoMusicNavigationInstance;
...
// 导出给全局使用
window.AutoMusicLoveNavigation = AutoMusicLoveNavigation;